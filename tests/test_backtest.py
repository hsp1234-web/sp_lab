...
def test_backtest_with_simple_cost():
    """
    測試案例 6：整合 simple cost model 的回測

    驗證：
    1. trade_log 中的 fees 欄位是否被正確計算。
    2. pnl 欄位是否能正確地扣除 fees。
    """
    price_data, signals = setup_basic_test_data()

    cost_params = {"fees_per_unit": 10.0, "slippage_per_unit": 1.0} # cost = 11 per trade (entry/exit)
    total_cost_per_trade = (10.0 + 1.0) * 1 * 2 # pos_size=1, *2 for entry and exit

    # 預期結果
    # Long trade: pnl = 10 (gross), fees = 22, net_pnl = 10 - 22 = -12
    # Short trade: pnl = 5 (gross), fees = 22, net_pnl = 5 - 22 = -17
    expected_log_data = [
        {
            "trade_id": 1, "entry_date": pd.to_datetime("2025-01-02"),
            "exit_date": pd.to_datetime("2025-01-03"), "direction": "Long",
            "entry_price": 110.0, "exit_price": 120.0, "quantity": 1,
            "pnl": -12.0, "realized_pnl": -12.0, "fees": 22.0, "notes": ""
        },
        {
            "trade_id": 2, "entry_date": pd.to_datetime("2025-01-06"),
            "exit_date": pd.to_datetime("2025-01-07"), "direction": "Short",
            "entry_price": 140.0, "exit_price": 135.0, "quantity": 1,
            "pnl": -17.0, "realized_pnl": -29.0, "fees": 22.0, "notes": ""
        }
    ]
    expected_trade_log = pd.DataFrame(expected_log_data)
