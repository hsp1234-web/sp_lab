...
2. 使用 FinMind API 下載美元兌新台幣 (USDTWD) 的歷史匯率數據。
3. 爬取台灣證券交易所的融資維持率數據。
4. 使用 FinMind API 下載三大法人買賣超數據。

執行方式：
在執行前，可選擇性設定您的 FinMind API 金鑰以獲得更穩定的服務：
export FINMIND_API_TOKEN='您的金鑰'
...
def fetch_forex_data_with_finmind(start_date, end_date):
    """使用 FinMind API 下載美元兌台幣匯率數據。"""
    logging.info("開始使用 FinMind API 下載 USDTWD 匯率數據...")

    # 登入 FinMind
    # 在實際應用中，建議將 token 儲存在環境變數中
    finmind_api_token = os.getenv('FINMIND_API_TOKEN', '')
    if not finmind_api_token:
        logging.warning("未設定 FINMIND_API_TOKEN 環境變數，將以未登入狀態請求。")
    ...
    try:
        ...
        if not data.get('data'):
            raise ValueError("FinMind API 未返回任何匯率數據。")

        df = pd.DataFrame(data['data'])
        df = df.rename(columns={'date': 'Date', 'spot_sell': 'Close'})
        df['Date'] = pd.to_datetime(df['Date'])
        df = df.set_index('Date').sort_index()

        # FinMind API 在特定情況下可能返回 -99，代表無數據，需要過濾
        df = df[df['Close'] > 0]
        ...
        logging.error(f"請求 FinMind API 時發生網路錯誤: {e}")
        return False
    except Exception as e:
        logging.error(f"處理 FinMind 匯率數據時發生錯誤: {e}", exc_info=True)
        return False
...
def fetch_institutional_investors_data(start_date, end_date):
    """使用 FinMind API 下載三大法人買賣超數據。"""
    logging.info("開始使用 FinMind API 下載三大法人數據...")
    ...
    try:
        ...
        if not data.get('data'):
            raise ValueError("FinMind API 未返回任何三大法人數據。")
        ...
        logging.error(f"處理 FinMind 三大法人數據時發生錯誤: {e}", exc_info=True)
        return False
...
def main():
    ...
    # 步驟 2: 下載匯率數據
    if not fetch_forex_data_with_finmind(args.start, args.end):
        logging.error("匯率數據下載失敗，流程中止。")
        return

    # 步驟 3: 爬取融资维持率数据
    if not fetch_twse_margin_data(args.start, args.end):
        logging.warning("融资维持率数据爬取失败，将继续执行。")

    # 步驟 4: 下載三大法人數據
    if not fetch_institutional_investors_data(args.start, args.end):
        logging.error("三大法人數據下載失敗，流程中止。")
        return
